  ...
  async invoke<T>(command: string, args?: CommandArgs): Promise<T> {
    ...
    switch (command) {
      ...
      case "auth_login": {
        ...
        if (!users.length) {
          throw new Error("Autenticação local desativada no Web/PWA. Configure o provider da API interna/remota.");
        }
      ...
      case "auth_change_password":
        throw new Error(
          "A troca de senha no Web/PWA exige conexão autenticada com a API interna do Ponto Manager.",
        );
      ...
      case "user_policy_get":
        requireSession(getString(args, "session_token"));
        return { password_min_length: 12, require_strong_password: true, session_timeout_minutes: 480 } as T;
      ...
      case "licensing_status":
        return { enabled: false, status: "disabled", message: "Licenciamento desativado no Ponto Manager Web." } as T;
      ...
      case "runtime_settings_load": {
        ...
        const settings = raw ? JSON.parse(raw) : {
          ...
          app_service_name: "ArgwsPontoManagerServer",
        };
      ...
      case "websocket_broadcast_test":
        return { ok: true, sent: 0, simulated: true, payload: { type: "ponto-manager.broadcast.test", message: getString(args, "message", "Teste Web/PWA"), sent_at: new Date().toISOString() } } as T;
