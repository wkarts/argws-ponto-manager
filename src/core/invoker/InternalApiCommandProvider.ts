...
type PontoManagerRuntimeWindow = Window & {
  __TAURI_INTERNALS__?: unknown;
  ARGWS_PONTO_MANAGER_WEB_RUNTIME?: boolean;
  ARGWS_PONTO_MANAGER_INTERNAL_API_WEB_RUNTIME?: boolean;
  ARGWS_PONTO_MANAGER_COMMAND_PROXY_BASE?: string;
  ARGWS_PONTO_MANAGER_API_BASE_URL?: string;
};

function runtimeWindow(): PontoManagerRuntimeWindow | null {
  if (typeof window === "undefined") return null;
  return window as PontoManagerRuntimeWindow;
}
...
function apiBaseUrl(): string {
  ...
  const proxyBase = win.ARGWS_PONTO_MANAGER_COMMAND_PROXY_BASE;
  if (proxyBase && proxyBase.trim()) {
    return trimRightSlash(proxyBase.trim());
  }

  const configured = win.ARGWS_PONTO_MANAGER_API_BASE_URL;
...
export function isInternalApiWebRuntime(): boolean {
  ...
  return Boolean(win.ARGWS_PONTO_MANAGER_INTERNAL_API_WEB_RUNTIME || win.ARGWS_PONTO_MANAGER_WEB_RUNTIME || port === "61002");
}
