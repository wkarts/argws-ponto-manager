...
import { baixarAfdConector as baixarAfdConectorApi, coletarBatidasConector, testarConector } from "../services/conectorService";
...
const message = ref("");
const error = ref("");
const equipamentoConectorId = ref<number | null>(null);
...
async function coletarData() {
  if (!equipamentoConectorId.value) return;
  const res = await coletarBatidasConector({
    equipamento_id: equipamentoConectorId.value,
...
async function baixarAfdArquivo() {
  if (!equipamentoConectorId.value) return;
  const data = await baixarAfdConectorApi(equipamentoConectorId.value);
...
<template>
  <div class="grid page-gap">
    ...
    <div class="card grid page-gap">
      ...
      <div class="grid columns-3 mobile-columns-1">
        <div class="field">
          <label>ID do Equipamento (Conector)</label>
          <input v-model.number="equipamentoConectorId" type="number" placeholder="Informe o equipamento_id" />
        </div>
        <div class="actions align-end">
          ...
          <button class="secondary" @click="baixarAfdArquivo">Baixar AFD</button>
        </div>
      </div>
