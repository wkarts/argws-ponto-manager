...
function exportCustomThemes() {
  ...
  a.href = url;
  a.download = `argws-ponto-manager-temas-${userVisualKey.value || "usuario"}.json`;
  a.click();
...
onMounted(async () => {
  ...
  window.addEventListener("ponto-manager:workspace-tabs-preference-changed", handleWorkspacePreferenceChanged as EventListener);
  ensureActiveGroup();
  closeSidebarOnMobile();
  upsertCurrentTab();
  scheduleTabScrollStateUpdate();
...
onBeforeUnmount(() => {
  ...
  if (typeof window !== "undefined") {
    ...
    window.removeEventListener("ponto-manager:workspace-tabs-preference-changed", handleWorkspacePreferenceChanged as EventListener);
  }
});
</script>
