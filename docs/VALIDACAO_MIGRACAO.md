...
- `cargo fmt`, `cargo check`, Clippy `-D warnings` e `cargo test` nativos Linux no GitHub Actions;
...
- check/test nativos Linux: o runtime local não fornece `pkg-config`, GTK e WebKitGTK, mas o mesmo gate foi aprovado no runner Ubuntu 24.04;
