...
impl WebSocketConfig {
    ...
    pub fn validate_bind(&self) -> Result<(), String> {
        ...
        if is_public && !self.allow_lan && !self.allow_external {
            return Err(
                "WebSocket público exige autorização explícita para LAN ou rede externa."
                    .to_string(),
            );
        }
        if is_public && !self.token_required {
            return Err("WebSocket público exige autenticação por token.".to_string());
        }
        if self.token_required && self.token.trim().is_empty() {
            return Err("WebSocket protegido exige token não vazio.".to_string());
        }
