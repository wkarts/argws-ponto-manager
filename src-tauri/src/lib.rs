...
pub fn run() {
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::conector::conector_configuracao_carregar,
            commands::conector::conector_configuracao_salvar,
            commands::conector::conector_importar_afd,
...
fn start_runtime_services_background(state: SharedState) {
    std::thread::Builder::new()
        .name("argws-ponto-manager-runtime-services-orchestrator".to_string())
        .spawn(move || {
...
fn is_tray_enabled() -> bool {
    env_bool("ARGWS_PONTO_MANAGER_TRAY_ENABLED", true)
}

#[cfg(feature = "desktop")]
fn close_to_tray() -> bool {
    env_bool("ARGWS_PONTO_MANAGER_TRAY_CLOSE_TO_TRAY", false)
}
...
fn should_start_internal_api(state: &SharedState) -> bool {
    ...
    settings.internal_api_auto_start
        && !env_bool("ARGWS_PONTO_MANAGER_API_DISABLED", false)
        && !env_bool("APP_API_DISABLED", false)
        && !env_bool("ARGWS_PONTO_MANAGER_INTERNAL_API_DISABLED", false)
}
