...
pub async fn company_lookup_cnpj(
    ...
    let publica_url = format!("{}{}", publica_base.trim_end_matches('/'), format!("/{}", digits));
    match fetch_json_text(publica_url).await.and_then(|body| parse_publica_ws_response(&body, uf_hint.as_deref())) {
        Ok(result) => return Ok(result),
        Err(err) => last_error = err,
    }

    let receita_url = format!("{}{}", receita_base.trim_end_matches('/'), format!("/{}", digits));
    match fetch_json_text(receita_url).await.and_then(|body| parse_receita_ws_response(&body)) {
        Ok(result) => Ok(result),
        Err(err) => Err(format!("Falha ao consultar CNPJ nos serviços públicos. Último erro: {}; erro anterior: {}", err, last_error)),
    }
}
