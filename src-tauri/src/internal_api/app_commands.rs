...
fn runtime_app_short_name() -> String {
    std::env::var("ARGWS_PONTO_MANAGER_SHORT_NAME")
        .or_else(|_| std::env::var("APP_SHORT_NAME"))
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "Ponto".to_string())
}
...
fn provider_for_state(state: &InternalApiState) -> Result<ActiveDatabaseProvider, String> {
    let driver = std::env::var("ARGWS_PONTO_MANAGER_DATABASE_DRIVER")
...
fn default_runtime_settings(state: &InternalApiState) -> Value {
    ...
    settings.insert(
        ...
        Value::String("ArgwsPontoManagerServer".to_string()),
    );
...
fn auth_change_password_http(
    ...
    if identity.master_user {
        crate::bootstrap::remove_after_password_change(&state.db_path)?;
    }
    Ok(Value::Bool(true))
}
...
pub(crate) fn execute_command(
    ...
    match command {
        ...
        "user_policy_get" => Ok(
            json!({"login_min_length": 3, "login_min_allowed": 3, "login_max_allowed": 60, "password_min_length": 12, "require_strong_password": true}),
        ),
        ...
        "user_save" => {
            ...
            if id <= 0 && senha.is_empty() {
                payload.insert(
                    ...
                    Value::String(hash_password("123456")?),
                );
        ...
        "websocket_broadcast_test" => {
            let payload = json!({"type": "ponto-manager.broadcast.test", "message": get_string(&args, "message"), "sent_at": now()});
