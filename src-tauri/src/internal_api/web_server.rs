...
pub fn start_background(state: SharedState) -> Result<WebRuntimeStatus, String> {
    ...
    let thread = spawn_web_server_thread(WebServerThreadConfig {
        name: "argws-ponto-manager-web-proxy".to_string(),
        state: state.clone(),
        host: host.clone(),
        port: bound_port,
        std_listener,
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let result = rt.block_on(async move {
                ...
                println!("Servidor web Ponto Manager ativo em http://{addr} desde {started_at}");
...
fn prepare_loopback_mirror(
    ...
    match spawn_web_server_thread(WebServerThreadConfig {
        name: "argws-ponto-manager-web-proxy-loopback".to_string(),
        state: state.clone(),
...
async fn web_health() -> Response {
    let body = serde_json::json!({
        "ok": true,
        "service": "argws-ponto-manager-web-proxy",
        "version": env!("CARGO_PKG_VERSION"),
        "web_port": web_port_from_env(),
        "api_base_url": normalize_client_base_url(
            &std::env::var("ARGWS_PONTO_MANAGER_API_BASE_URL")
...
fn web_bind_host_from_env() -> String {
    std::env::var("ARGWS_PONTO_MANAGER_WEB_HOST")
...
fn web_port_from_env() -> u16 {
    std::env::var("ARGWS_PONTO_MANAGER_WEB_PORT")
...
fn adopt_existing_web_runtime(bind_host: &str, port: u16) -> Option<WebRuntimeStatus> {
    ...
    Some(WebRuntimeStatus {
        ...
        started_at: None,
        external: true,
        message: Some("Webport já estava ativo em outra instância do Ponto Manager; esta janela reutilizou a instância existente sem tentar abrir uma segunda porta.".to_string()),
    })
}
...
fn probe_template_webport(client: &reqwest::blocking::Client, url: &str) -> bool {
    ...
    if let Ok(value) = serde_json::from_str::<serde_json::Value>(&body) {
        if value
            ...
            .map(|value| {
                value == "argws-ponto-manager-web-proxy" || value == "internal-api-via-webport"
            })
            .unwrap_or(false)
        {
            return true;
    ...
    normalized.contains("ponto manager")
        || normalized.contains("argws-ponto-manager-web-proxy")
        || normalized.contains("argws_ponto_manager_web_runtime")
}
