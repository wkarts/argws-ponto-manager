...
pub fn router(state: InternalApiState) -> Router {
    ...
    if state.config.security.cors_enabled {
        let allowed_origins = state
            ...
            .collect::<Vec<_>>();
        let token_header = HeaderName::from_bytes(state.config.security.token_header.as_bytes())
            .unwrap_or_else(|_| HeaderName::from_static("x-app-token"));
        router.layer(
            CorsLayer::new()
...
async fn token_guard(
    ...
    if configured_token.is_empty() {
        ...
        return (StatusCode::SERVICE_UNAVAILABLE, Json(json!({
            ...
            "hint": "Defina ARGWS_PONTO_MANAGER_API_TOKEN ou desative ARGWS_PONTO_MANAGER_API_REQUIRE_TOKEN apenas em ambiente local controlado."
        })))
        .into_response();
    }
...
async fn status(State(state): State<InternalApiState>) -> Json<Value> {
    Json(json!({
        ...
        "security": {
            ...
            "cors_allowed_origins": state.config.security.cors_allowed_origins,
