...
impl Default for InternalApiConfig {
    fn default() -> Self {
        let host =
            env::var("ARGWS_PONTO_MANAGER_API_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
        let port = env::var("ARGWS_PONTO_MANAGER_API_PORT")
            .ok()
            .and_then(|value| value.parse::<u16>().ok())
            .unwrap_or(61001);
        let docs_path = normalize_docs_path(
            &env::var("ARGWS_PONTO_MANAGER_API_DOCS_PATH").unwrap_or_else(|_| "/docs".to_string()),
        );
        let expose_docs = env_bool("ARGWS_PONTO_MANAGER_API_SCALAR_ENABLED", true)
            && env_bool("ARGWS_PONTO_MANAGER_API_EXPOSE_DOCS", true);
        let require_token = env_bool("ARGWS_PONTO_MANAGER_API_REQUIRE_TOKEN", true);
        let allow_public_network = env_bool("ARGWS_PONTO_MANAGER_API_ALLOW_PUBLIC_NETWORK", false);
        let cors_enabled = env_bool("ARGWS_PONTO_MANAGER_API_CORS", false);
        let cors_allowed_origins = env::var("ARGWS_PONTO_MANAGER_API_CORS_ORIGINS")
            .unwrap_or_else(|_| "http://127.0.0.1:61002,http://localhost:61002".to_string())
            .split(',')
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(str::to_string)
            .collect::<Vec<_>>();
        let docs_public = env_bool("ARGWS_PONTO_MANAGER_API_DOCS_PUBLIC", false);
        let docs_public_local = env_bool("ARGWS_PONTO_MANAGER_API_DOCS_PUBLIC_LOCAL", true);
        let token_header = env::var("ARGWS_PONTO_MANAGER_API_TOKEN_HEADER")
            .unwrap_or_else(|_| "X-App-Token".to_string());
        let token = env::var("ARGWS_PONTO_MANAGER_API_TOKEN")
            .ok()
            .filter(|value| !value.trim().is_empty());
        let base_url = env::var("ARGWS_PONTO_MANAGER_API_BASE_URL")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| format!("http://{host}:{port}"));
        let docs_url = env::var("ARGWS_PONTO_MANAGER_API_SCALAR_URL")
            .ok()
            .filter(|value| !value.trim().is_empty())
        ...
        Self {
            enabled: true,
            auto_start: env_bool("ARGWS_PONTO_MANAGER_API_AUTO_START", false),
            host: host.clone(),
            port,
            base_url: base_url.clone(),
            timeout_ms: env_u64("ARGWS_PONTO_MANAGER_API_TIMEOUT_MS", 8000),
            log_mode: env::var("ARGWS_PONTO_MANAGER_API_LOG_MODE")
                .unwrap_or_else(|_| "normal".to_string()),
            open_scalar_after_start: env_bool(
                "ARGWS_PONTO_MANAGER_API_OPEN_SCALAR_AFTER_START",
                false,
            ),
            restart_on_config_change: env_bool(
...
impl InternalApiConfig {
    ...
    pub fn validate_bind(&self) -> Result<(), String> {
        ...
        if is_public && !self.security.allow_public_network {
            return Err("API interna bloqueada: bind público exige ARGWS_PONTO_MANAGER_API_ALLOW_PUBLIC_NETWORK=true.".to_string());
        }
        if is_public && !self.security.require_token {
            return Err(
                "API interna bloqueada: bind público exige autenticação por token.".to_string(),
            );
        }
        if self.security.require_token
            && self
                .security
                .token
                .as_deref()
        ...
        {
            return Err(
                "API interna protegida exige ARGWS_PONTO_MANAGER_API_TOKEN não vazio.".to_string(),
            );
        }
        Ok(())
    }
