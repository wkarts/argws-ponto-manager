...
fn resolve_dist_dir() -> Option<PathBuf> {
    ...
    if let Ok(value) = env::var("ARGWS_PONTO_MANAGER_WEB_DIST_DIR") {
    ...
    if let Ok(exe_path) = env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            ...
            candidates.push(exe_dir.join("../dist"));
            candidates.push(exe_dir.join("../Resources/dist"));
            candidates.push(exe_dir.join("../share/argws-ponto-manager/dist"));
        }
    }
...
window.ARGWS_PONTO_MANAGER_WEB_RUNTIME = true;
window.ARGWS_PONTO_MANAGER_INTERNAL_API_WEB_RUNTIME = true;
window.ARGWS_PONTO_MANAGER_COMMAND_PROXY_BASE = '/__internal_api';
</script>"#;
    ...
    if !output.contains("ARGWS_PONTO_MANAGER_INTERNAL_API_WEB_RUNTIME") {
...
<body>
  <main><section class="card">
    ...
    <p>Execute <code>npm run build:web</code> antes do build Tauri, ou defina <code>ARGWS_PONTO_MANAGER_WEB_DIST_DIR</code> apontando para a pasta <code>dist</code>.</p>
  </section></main>
</body>
</html>"#,
        mount_path
