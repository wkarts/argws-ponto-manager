...
pub async fn run_with_config(state: &SharedState, config: InternalApiConfig) -> Result<(), String> {
    ...
    std::env::set_var("ARGWS_PONTO_MANAGER_API_HOST", config.host.clone());
    std::env::set_var("ARGWS_PONTO_MANAGER_API_PORT", config.port.to_string());
    std::env::set_var(
...
pub fn start_background(
    ...
    let addr = socket_addr(&config)?;
    std::env::set_var("ARGWS_PONTO_MANAGER_API_HOST", host.clone());
    std::env::set_var("ARGWS_PONTO_MANAGER_API_PORT", port.to_string());
    std::env::set_var(
        "ARGWS_PONTO_MANAGER_API_BASE_URL",
    ...
    let std_listener = match StdTcpListener::bind(addr) {
        ...
        Err(err) => {
            ...
            let message = format!(
                "Porta da API interna {addr} indisponível: {err}. A porta está ocupada por outro processo que não respondeu como Ponto Manager API. Altere ARGWS_PONTO_MANAGER_API_PORT ou encerre o processo externo que está usando esta porta."
            );
            log_runtime_event(
                &state,
                "error",
    ...
    let thread = std::thread::Builder::new()
        .name("argws-ponto-manager-internal-api".to_string())
        .spawn(move || {
...
pub fn status() -> Result<ApiRuntimeStatus, String> {
    ...
    if let Some(handle) = guard.as_ref() {
        ...
        let host = std::env::var("ARGWS_PONTO_MANAGER_API_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string());
        let public_host = crate::internal_api::config::public_url_host(&host);
        let port = std::env::var("ARGWS_PONTO_MANAGER_API_PORT")
            .ok()
...
fn adopt_existing_api_runtime(public_host: &str, port: u16) -> Option<ApiRuntimeStatus> {
    ...
    Some(ApiRuntimeStatus {
        ...
        started_at: None,
        external: true,
        message: Some("API interna já estava ativa em outra instância do Ponto Manager; esta janela reutilizou a instância existente sem tentar abrir uma segunda porta.".to_string()),
    })
}
