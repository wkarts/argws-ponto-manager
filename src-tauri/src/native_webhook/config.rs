...
impl WebhookConfig {
    ...
    pub fn validate_bind(&self) -> Result<(), String> {
        ...
        if is_public && !self.allow_lan && !self.allow_external {
            return Err(
                "Webhook público exige autorização explícita para LAN ou rede externa.".to_string(),
            );
        }
        if is_public && !self.token_required {
            return Err("Webhook público exige autenticação por token.".to_string());
        }
        if self.token_required && self.token.trim().is_empty() {
            return Err("Webhook protegido exige token não vazio.".to_string());
        }
