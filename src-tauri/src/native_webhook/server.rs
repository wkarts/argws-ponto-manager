...
pub fn start_background_with_config(
    ...
    let thread = spawn_webhook_server_thread(WebhookServerThreadConfig {
        name: "argws-ponto-manager-webhook-service".to_string(),
        state: state.clone(),
        std_listener,
        config: config.clone(),
        rx,
...
pub fn status() -> Result<WebhookRuntimeStatus, String> {
    ...
    if let Some(handle) = guard.as_ref() {
        ...
        Ok(WebhookRuntimeStatus {
            running: false,
            host: std::env::var("ARGWS_PONTO_MANAGER_WEBHOOK_HOST")
                .unwrap_or_else(|_| "0.0.0.0".to_string()),
            port: std::env::var("ARGWS_PONTO_MANAGER_WEBHOOK_PORT")
                .ok()
                .and_then(|value| value.parse::<u16>().ok())
                .unwrap_or(61003),
            url: None,
            base_path: std::env::var("ARGWS_PONTO_MANAGER_WEBHOOK_BASE_PATH")
...
fn prepare_loopback_mirror(
    ...
    match spawn_webhook_server_thread(WebhookServerThreadConfig {
        name: "argws-ponto-manager-webhook-service-loopback".to_string(),
        state: state.clone(),
...
fn bind_listener_with_safe_fallback(
    ...
    crate::net_bind::bind_fixed_service_listener(&config.host, config.port).map_err(|err| {
        format!(
            "Porta fixa do Webhook Service {requested_addr} indisponível: {err}. Ajuste ARGWS_PONTO_MANAGER_WEBHOOK_PORT/parâmetros ou encerre o processo que está usando esta porta."
        )
    })
}
