...
impl DatabaseConfig {
    pub fn from_env_with_driver(driver: &str) -> Self {
        ...
        let prefix = match parsed_driver {
            DatabaseDriver::Mysql => "ARGWS_PONTO_MANAGER_MYSQL",
            DatabaseDriver::Postgres => "ARGWS_PONTO_MANAGER_POSTGRES",
            DatabaseDriver::Sqlite | DatabaseDriver::FirebirdUnsupported => {
                "ARGWS_PONTO_MANAGER_DB"
            }
        };
        ...
        Self {
            ...
            sqlite_path: env::var("ARGWS_PONTO_MANAGER_SQLITE_PATH")
                .ok()
                .or_else(|| Some("app.db".to_string())),
            host: env::var(format!("{prefix}_HOST"))
                .ok()
                .or_else(|| env::var("ARGWS_PONTO_MANAGER_DB_HOST").ok()),
            port: env::var(format!("{prefix}_PORT"))
                .ok()
                .or_else(|| env::var("ARGWS_PONTO_MANAGER_DB_PORT").ok())
                .and_then(|value| value.parse::<u16>().ok())
                .or(default_port),
            database: env::var(format!("{prefix}_DATABASE"))
                .ok()
                .or_else(|| env::var("ARGWS_PONTO_MANAGER_DB_DATABASE").ok()),
            username: env::var(format!("{prefix}_USERNAME"))
                .ok()
                .or_else(|| env::var("ARGWS_PONTO_MANAGER_DB_USERNAME").ok()),
            password: env::var(format!("{prefix}_PASSWORD"))
                .ok()
                .or_else(|| env::var("ARGWS_PONTO_MANAGER_DB_PASSWORD").ok()),
        }
    }
